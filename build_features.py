...
# ファイル名は集約先の実ファイル名（旧・西宮表記）で固定し、article_href()がローカルに実ファイルが
# あるサイト（西宮）は相対リンク、無いサイト（複製都市）は集約先への絶対URLを返す。
